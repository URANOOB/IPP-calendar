...
export default async function DashboardPage() {
  ...
  const supabase = await createClient(); const cycle = overview.currentOpenCycle ?? overview.upcomingCycle; const cycleId = cycle?.id;
  ...
  const [{ count: guardianCount }, { count: contactedCount }, { count: bookedCount }, { count: registrationCount }, { data: classes }, { data: teachers }, { data: cycles }, { data: registrations }] = results;
  ...
  return (
    <>
      ...
      <section className="relative overflow-hidden rounded-2xl border border-cyan-100 bg-gradient-to-r from-cyan-50 via-white to-indigo-50 px-6 py-7 text-sm text-muted-foreground shadow-[0_12px_28px_rgba(59,130,246,0.08)]">
        ...
        {cycle ? <><p className="font-semibold text-foreground">{overview.currentOpenCycle ? "Ciclo actual" : "Próximo ciclo"}: {cycle.name}</p><p className="mt-2">{getCycleEffectiveStatus(cycle)}</p></> : "No hay un ciclo activo."}
        </div>
      </section>
