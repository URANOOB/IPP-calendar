...
const responseLabels: Record<string, string> = { not_contacted: "Pendiente", contacted: "Contactado", no_response: "Sin respuesta", interested: "Interesado", declined: "No interesado", booked: "Agendado", registered: "Registrado" };
...
const csvCell = (value: string) => { const safe = /^[=+@\-\t\r]/.test(value) ? `'${value}` : value; return `"${safe.replace(/"/g, '""')}"`; };
...
export async function GET(request: NextRequest) {
  ...
  if (results.some((result) => result.error)) return new Response("No fue posible exportar los datos. Intenta nuevamente.", { status: 503 });
  const [{ data: registrations }, { data: students }, { data: guardians }, { data: tracking }, { data: classes }, { data: teachers }, { data: profiles }] = results;
  const rows = (guardians ?? []).flatMap((guardian) => { const track = tracking?.find((item) => item.guardian_id === guardian.id); const manager = profiles?.find((item) => item.id === track?.assigned_to); const guardianStudents = students?.filter((item) => item.guardian_id === guardian.id) ?? []; const exportStudents = guardianStudents.length ? guardianStudents : [{ id: "", full_name: "" }]; return exportStudents.flatMap((student) => { const entries = student.id ? (registrations ?? []).filter((registration) => registration.student_id === student.id) : []; const exportEntries = entries.length ? entries : [null]; return exportEntries.map((registration) => { const classItem = registration ? classes?.find((item) => item.id === registration.class_id) : null; const teacher = classItem ? teachers?.find((item) => item.id === classItem.teacher_id) : null; return [guardian.full_name ?? "", guardian.phone, student.full_name, manager?.full_name ?? (track?.assigned_to ? "Otro gestor" : "Sin responsable"), track?.first_contact_at ? formatBogotaDateTime(track.first_contact_at) : "", track?.invitation_sent_at ? formatBogotaDateTime(track.invitation_sent_at) : "", responseLabels[track?.response_status ?? "not_contacted"] ?? "Pendiente", registration ? "Sí" : "No", classItem?.title ?? "", teacher?.display_name ?? "", classItem ? formatBogotaDateTime(classItem.starts_at) : "", registration?.status === "attended" ? "Asistió" : registration?.status === "absent" ? "No asistió" : ""]; }); }); });
