...
export default async function ClassesPage() {
  ...
  const results = await Promise.all([supabase.from("classes").select("*").order("starts_at"), supabase.from("teachers").select("id, display_name, active"), supabase.from("weekly_cycles").select("id, name, status, starts_at, ends_at").order("starts_at"), supabase.from("registrations").select("class_id").in("status", ["pending", "confirmed", "attended", "absent"])]);
  if (results.some((result) => "error" in result && result.error)) throw new Error("No fue posible cargar los datos de esta sección.");
  const [{ data: classes }, { data: teachers }, { data: cycles }, { data: registrations }] = results;
  ...
  const teacherOptions: ClassTeacherOption[] = (teachers ?? []).filter((teacher) => teacher.active).map((teacher) => ({ id: teacher.id, name: teacher.display_name })); const cycleOptions: ClassCycleOption[] = (cycles ?? []).filter((cycle) => cycle.status === "open").map((cycle) => ({ id: cycle.id, name: cycle.name, startsAt: cycle.starts_at, endsAt: cycle.ends_at }));
  return <><PageHeader title="Clases" description="Gestiona la oferta de clases de cada ciclo semanal." /><ClassesManager canDelete={user.role === "admin"} classes={list} cycles={cycleOptions} teachers={teacherOptions} /></>;
}
