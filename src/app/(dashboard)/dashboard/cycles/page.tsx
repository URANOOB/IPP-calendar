...
export default async function CyclesPage() {
  const user = await requireDashboardRoute("/dashboard/cycles");
  ...
  return <><PageHeader title="Ciclos semanales" description="Crea semanas independientes y controla sus ventanas de inscripción." />
    {overview.currentOpenCycle ? <section className="mb-6 rounded-xl border bg-card p-5"><p className="text-sm font-semibold text-primary">Ciclo operativo actual</p><h2 className="mt-1 text-xl font-bold">{overview.currentOpenCycle.name}</h2><p className="mt-2 text-sm text-muted-foreground">{getCycleEffectiveStatus(overview.currentOpenCycle)}</p></section> : <section className="mb-6 rounded-xl border border-dashed bg-card p-5 text-sm text-muted-foreground">{overview.upcomingCycle ? `Próximo ciclo activo: ${overview.upcomingCycle.name}` : "No hay ciclos activos. Crea el primero para comenzar."}</section>}
    <CyclesManager canDelete={user.role === "admin"} cycles={listItems} /></>;
}
