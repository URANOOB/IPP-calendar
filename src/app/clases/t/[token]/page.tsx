...
export default async function PrivateClassAccessPage({ params }: Readonly<{ params: Promise<{ token: string }> }>) {
  ...
  return <main className="min-h-screen px-4 py-8 sm:px-6 sm:py-12"><div className="mx-auto w-full max-w-3xl"><header className="mb-8 rounded-2xl bg-primary p-6 text-primary-foreground sm:p-8"><BookOpenCheck aria-hidden="true" className="mb-4 size-10" /><h1 className="text-3xl font-bold">{context.guardian_name ? `Hola, ${context.guardian_name}` : "Hola"}</h1><p className="mt-3 text-lg leading-7 text-primary-foreground/90">Consulta y programa las clases de tus niños.</p></header><PrivateLinkCopy origin={origin} token={parsed.data} /><WaitingRoom classes={waitingClasses} serverNow={new Date().toISOString()} token={parsed.data} />{!hasCycle ? <section className="rounded-2xl border bg-card p-6 text-lg shadow-sm">Las inscripciones aún no están disponibles.</section> : allStudentsRegistered ? null : !context.registration_open ? <><section className="mb-6 rounded-2xl border bg-card p-6 text-lg shadow-sm">{context.cycle_status === "closed" ? "Las inscripciones de esta semana ya finalizaron." : "Las inscripciones aún no están disponibles."}</section><RegistrationFlow canRegister={false} classes={classes} students={students} token={parsed.data} /></> : <RegistrationFlow canRegister classes={classes} students={students} token={parsed.data} />}</div></main>;
}
