...
function WaitingRoomCard({ token, classItem, serverNow }: { token: string; classItem: WaitingRoomClass; serverNow: string }) {
  ...
  const message = state === "cancelled" ? "Esta clase fue cancelada." : state === "finished" ? "Esta clase ya finalizó." : state === "live" ? "La clase está en curso." : state === "starting_soon" ? "Tu clase comenzará pronto." : "El enlace estará disponible 30 minutos antes.";
  return <article className="rounded-2xl border bg-card p-5 shadow-sm sm:p-6"><p className="text-xl font-bold">{classItem.studentName}</p><h3 className="mt-3 text-2xl font-bold">{classItem.title}</h3><p className="mt-1 text-lg">Prof. {classItem.teacherName}</p><p className="mt-3 text-lg text-muted-foreground">{formatBogotaDate(classItem.startsAt)}<br />{time(classItem.startsAt)} – {time(classItem.endsAt)}</p>{state === "upcoming" || state === "starting_soon" ? <Countdown serverNow={serverNow} startsAt={classItem.startsAt} /> : <p className="mt-3 text-lg font-bold text-primary">{message}</p>}{state === "cancelled" || state === "finished" ? <p className="mt-3 text-lg font-bold text-muted-foreground">{message}</p> : null}{showMeeting ? <div className="mt-5">{meetingUrl ? <Button asChild className="h-13 w-full text-lg sm:w-auto sm:px-7"><a href={meetingUrl} rel="noreferrer" target="_blank"><Video />Entrar a la clase<ExternalLink /></a></Button> : <p className="text-base text-muted-foreground">{pending ? "Preparando acceso…" : error ?? "El enlace no está disponible todavía."}</p>}</div> : null}</article>;
}
