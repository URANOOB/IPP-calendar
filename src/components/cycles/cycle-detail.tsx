...
export function CycleDetail({ cycle }: Readonly<{ cycle: CycleDetailData }>) {
  const router = useRouter();
  const [error, setError] = useState<string>();
  const [isPending, startTransition] = useTransition();
  const form = useForm<WeeklyCycleValues>({
    resolver: zodResolver(weeklyCycleSchema),
    defaultValues: {
      name: cycle.name,
      startsAt: utcToBogotaInput(cycle.startsAt),
      endsAt: utcToBogotaInput(cycle.endsAt),
      registrationOpensAt: utcToBogotaInput(cycle.registrationOpensAt),
      registrationClosesAt: utcToBogotaInput(cycle.registrationClosesAt),
    },
  });
  ...
  return <div className="space-y-6">
    <Button asChild variant="outline"><Link href="/dashboard/cycles">Volver a ciclos</Link></Button>
    ...
    {cycle.status !== "draft" ? <section className="rounded-xl border bg-card p-5 text-sm text-muted-foreground">Las fechas de un ciclo {WEEKLY_CYCLE_STATUS_LABELS[cycle.status].toLowerCase()} están bloqueadas para preservar la operación y el histórico. Usa las acciones de estado desde el listado cuando corresponda.</section> : <form className="grid gap-4 rounded-xl border bg-card p-5 sm:grid-cols-2" noValidate onSubmit={form.handleSubmit(onSubmit)}>
    ...
    <section className="rounded-xl border bg-card p-5 text-sm text-muted-foreground"><p><strong className="text-foreground">Inicio:</strong> {formatBogotaDateTime(cycle.startsAt)}</p><p className="mt-2"><strong className="text-foreground">Fin:</strong> {formatBogotaDateTime(cycle.endsAt)}</p><p className="mt-2"><strong className="text-foreground">Ventana:</strong> {formatBogotaDateTime(cycle.registrationOpensAt)} — {formatBogotaDateTime(cycle.registrationClosesAt)}</p></section>
  </div>;
}
